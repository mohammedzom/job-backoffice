                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <div class="h-8 w-8 rounded-full bg-indigo-100 dark:bg-indigo-900/50 flex items-center justify-center text-indigo-600 dark:text-indigo-400 font-bold shadow-sm">
                                            {{ substr($jobApplication->user->name, 0, 1) }}
                                        </div>
                                        <div class="ml-3">
                                            @if (request('archived'))
                                                <span class="text-sm font-semibold text-gray-800 dark:text-gray-400 block">{{ $jobApplication->user->name }}</span>
                                            @else
                                            ...
                                            @endif
                                            <span class="text-xs text-gray-500 dark:text-gray-500 block">{{ $jobApplication->user->email }}</span>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700 dark:text-gray-300">
                                    <span class="inline-flex items-center px-2.5 py-1 rounded-md text-xs font-semibold bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300 border border-gray-200 dark:border-gray-600">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm">
                                    @php
                                        ...
                                        $colorClass = $statusColors[$jobApplication->status] ?? 'bg-gray-100 text-gray-800 dark:bg-gray-900/30 dark:text-gray-400 border border-gray-200 dark:border-gray-800';
                                    @endphp
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold shadow-sm {{ $colorClass }}">
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-6 py-16 text-center text-gray-500 dark:text-gray-400">
                                    ...
                                    <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100">{{ __('No applications found') }}</h3>
                                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ __('There are currently no job applications matching your criteria.') }}</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
