                        ...
                        @forelse ($companies as $company)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <a href="{{ route('company.show', $company->id) }}" class="text-sm font-semibold text-indigo-600 dark:text-indigo-400 hover:text-indigo-900 dark:hover:text-indigo-300 transition-colors">{{ $company->name }}</a>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700 dark:text-gray-300">
                                    {{ $company->address ?: __('Not specified') }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700 dark:text-gray-300">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm">
                                    @if($company->website)
                                        <a href="{{ $company->website }}" target="_blank" class="text-indigo-500 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-300 underline underline-offset-2 transition-colors">
                                    ...
                                    <div class="flex items-center justify-end space-x-3">
                                        @if (!request('archived'))
                                            <a href="{{ route('company.edit', $company->id) }}"
                                                class="text-indigo-600 dark:text-indigo-400 hover:text-indigo-900 dark:hover:text-indigo-300" title="Edit">
                                            ...
                                            <form action="{{ route('company.destroy', $company->id) }}" method="POST"
                                                ...
                                                @csrf
                                                @method('DELETE')
                                                <button class="text-red-500 dark:text-red-400 hover:text-red-700 dark:hover:text-red-300 transition-colors" type="submit" title="Archive" onclick="return confirm('{{ __('Are you sure you want to archive this company?') }}')">
                                        ...
                                        @else
                                            <form action="{{ route('company.restore', $company->id) }}" method="POST"
                                                ...
                                                @csrf
                                                @method('PUT')
                                                <button class="text-emerald-500 dark:text-emerald-400 hover:text-emerald-700 dark:hover:text-emerald-300 transition-colors" type="submit" title="Restore" onclick="return confirm('{{ __('Are you sure you want to restore this company?') }}')">
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-6 py-12 text-center text-gray-500 dark:text-gray-400">
                                    ...
                                    <p class="mt-2">{{ __('No companies found') }}</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
